...
def test_add_new_pet_with_valid_data(name='Дося', animal_type='безпороды',
                                     age='2', pet_photo='images/photo_2022-08-26_16-09-01.jpg'):
...
def test_successful_delete_self_pet():
    ...
    if len(my_pets['pets']) == 0:
        pf.add_new_pet(auth_key, "Досинья", "кошка", "3", "images/photo_2022-08-26_16-09-01.jpg")
        _, my_pets = pf.get_list_of_pets(auth_key, "my_pets")
...
def test_add_new_pet_with_negative_age(name='Дося', animal_type='безпороды',
                                       age='-100', pet_photo='images/photo_2022-08-26_16-09-01.jpg'):
    pet_photo = os.path.join(os.path.dirname(__file__), pet_photo)
    _, auth_key = pf.get_api_key(valid_email, valid_password)
    status, result = pf.add_new_pet(auth_key, name, animal_type, age, pet_photo)
    assert status == 200
...
def test_add_new_pet_with_invalid_pet_photo(name='Аксель', animal_type='безпороды',
                                            age='2', pet_photo='images/photo_2022-08-26_16-09-01.jpg'):
    _, auth_key = pf.get_api_key(valid_email, valid_password)
    status, result = pf.add_new_pet(auth_key, name, animal_type, age, pet_photo)
    assert status == 200
...
def test_add_new_pet_age_as_string(name='Дося', animal_type='безпороды',
                                   age='mjdfj', pet_photo='images/photo_2022-08-26_16-09-01.jpg'):
    pet_photo = os.path.join(os.path.dirname(__file__), pet_photo)
    _, auth_key = pf.get_api_key(valid_email, valid_password)
    status, result = pf.add_new_pet(auth_key, name, animal_type, age, pet_photo)
    assert status == 200
